...
def indexer_corpus_generique(
    ...
    """
    Fonction d'indexation avec provisionnement conditionnel de l'espace vectoriel.
    Architecture agnostique du schéma de données.
    """
    # 1. Vérification formelle et instanciation conditionnelle
    if not client.collection_exists(collection_name):
        client.create_collection(
    ...
    # 2. Chargement des structures de données en mémoire
    df_vecteurs = pd.read_parquet(vector_file_path, engine="pyarrow")
    df_metadata = pd.read_parquet(metadata_file_path, engine="pyarrow")
    
    # Casting strict des types pour la jointure matricielle
    df_vecteurs[join_key] = df_vecteurs[join_key].astype(int)
    df_metadata[join_key] = df_metadata[join_key].astype(int)
    
    # Intersection matricielle sur la clé primaire
    df_merged = pd.merge(df_vecteurs, df_metadata, on=join_key, how="inner")
    
    # 3. Déduction dynamique du schéma de payload
    if payload_columns is None:
        payload_columns = [col for col in df_merged.columns if col not in [join_key, vector_column]]

    points = []
    
    # 4. Parcours itératif et construction des structures
    for row in df_merged.itertuples(index=False):
        
        vector_val = getattr(row, vector_column)
        
        # Contrôle d'intégrité de l'embedding
        is_missing = pd.isna(vector_val).all() if isinstance(vector_val, (np.ndarray, list)) else vector_val is None
        if is_missing:
            continue
            
        # Résolution du vecteur
        vector_data = vector_val.tolist() if hasattr(vector_val, "tolist") else list(vector_val)

        # Génération dynamique du dictionnaire de payload
        payload_data = {}
        for col in payload_columns:
            ...
            if pd.notna(val):
                ...
                if isinstance(val, (np.bool_, bool)):
                    ...
                    payload_data[col] = float(val)
                else:
                    payload_data[col] = str(val)

        points.append(
            PointStruct(
                id=int(getattr(row, join_key)),
                vector=vector_data,
    ...
    # 5. Création des index de métadonnées conditionnels
    if index_fields:
